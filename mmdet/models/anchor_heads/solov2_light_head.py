...
class SOLOv2LightHead(nn.Module):
    ...
    def __init__(self,
        ...
        # ######################################## 20211209 添加1/4对应的特征图 ########################################
        self.add_4 = True if len(strides) == 6 else False  # 是否添加1/4对应的特征图, True or False
        # 1 ##((1, 32), (16, 64), (32, 128), (64, 256), (128, 512), (256, 2048))
        # 2 ##((1, 48), (16, 64), (32, 128), (64, 256), (128, 512), (256, 2048))
        # 3 ##((1, 48), (32, 64), (48, 128), (64, 256), (128, 512), (256, 2048))
        if self.add_4:
            strides = [4, 8, 8, 16, 32, 32]
            scale_ranges = ((1, 32), (16, 64), (32, 128), (64, 256), (128, 512), (256, 2048))
            num_grids = [52, 40, 36, 24, 16, 12]
        # ######################################## 20211209 添加1/4对应的特征图 ########################################
        ...
        # ################################################# ECA-Block ################################################
        # self.with_ECANet = ResNet(50).eca
        # if self.with_ECANet:
        #     ECA_planes = self.seg_feat_channels
        #     self.ECA_block = ECA_Layer(ECA_planes)
        # ################################################# ECA-Block ################################################
        self._init_layers()
    ...
    def forward(self, feats, eval=False):
        ...
        # ######################################## 20211209 添加1/4对应的特征图 ########################################
        if self.add_4:
            new_feats = (feats[0], new_feats[0], new_feats[1], new_feats[2], new_feats[3], new_feats[4])
        ...
        # # ############################################ 20211202 保存特征图 ############################################
        # for n, a in enumerate(new_feats):   # 测试的时候用
        #     import cv2
        #     import numpy as np
        #     seg_pred = a.cpu().squeeze()
        #     cur_mask = seg_pred.mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).type(torch.uint8).numpy()
        #     for m in range(cur_mask.shape[2]):
        #         cv2.imwrite("/media/wy/SOLO-master/train/1/{}_{}.jpg".format(str(n), str(m)), cur_mask[:, :, m])
        # # ############################################ 20211202 保存特征图 ############################################
    ...
    def forward_single(self, x, idx, eval=False, upsampled_size=None):
        ...
        for i, kernel_layer in enumerate(self.kernel_convs):
            ...
            # ############################################### ECA-Block ##############################################
            # if self.with_ECANet and i in [1, 2]:
            #     kernel_feat = self.ECA_block(kernel_feat)
            # ############################################### ECA-Block ##############################################
        ...
        for i, cate_layer in enumerate(self.cate_convs):
            ...
            # ############################################### ECA-Block ##############################################
            # if self.with_ECANet and i in [1, 2]:
            #     cate_feat = self.ECA_block(cate_feat)
            # ############################################### ECA-Block ##############################################
    ...
    def solov2_target_single(self,
        ...
        for (lower_bound, upper_bound), stride, num_grid \
            ...
            # # ######### 20211209 添加1/4对应的特征图并选取小物体实例进行卷积，对于没有小物体的特征图，任意选择一个实例 #########
            # if num_grid == self.seg_num_grids[0]:
            #     index = ((gt_labels == 1) | (gt_labels == 10)).nonzero().flatten()
            #     num_True = len(index)
            #     if num_True == 0:
            #         random_select = gt_labels.size()[0]
            #         index = (gt_labels == gt_labels[random.randint(0, random_select-1)]).nonzero().flatten()
            #         # index = (gt_labels == gt_labels[0]).nonzero().flatten()  # 每个epoch选定同一个实例
            #     gt_bboxes = gt_bboxes_raw[index]
            #     gt_labels = gt_labels_raw[index]
            #     gt_masks = gt_masks_raw[index.cpu().numpy(), ...]
            # # ######### 20211209 添加1/4对应的特征图并选取小物体实例进行卷积，对于没有小物体的特征图，任意选择一个实例 #########

            # # ######### 20211212 添加1/4对应的特征图并选取小物体实例进行卷积，对于没有小物体的特征图，选择所有实例卷积 #########
            # if num_grid == self.seg_num_grids[0]:
            #     index = ((gt_labels == 1) | (gt_labels == 10)).nonzero().flatten()
            #     num_True = len(index)
            #     if num_True == 0:
